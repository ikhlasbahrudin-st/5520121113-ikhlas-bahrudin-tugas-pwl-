    ...
    <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-4 px-4">
        <div>
            <h5 class="mb-1 fw-bold text-dark d-flex align-items-center gap-2">
                <i class="bi bi-mortarboard text-primary"></i> Manajemen Data Mahasiswa
            </h5>
            <p class="text-muted small mb-0">Kelola informasi data induk mahasiswa universitas.</p>
        </div>
        ...
        <div class="d-flex align-items-center gap-3">
            <div class="d-flex align-items-center gap-1 border-end pe-3">
                ...
                @else
                    <a href="{{ $mahasiswas->previousPageUrl() }}" class="btn btn-nav-arrow" title="Halaman Sebelumnya">
                        <i class="bi bi-chevron-left"></i>
                    </a>
                @endif
                
                @if($mahasiswas->hasMorePages())
                    <a href="{{ $mahasiswas->nextPageUrl() }}" class="btn btn-nav-arrow" title="Halaman Berikutnya">
            ...
            <a href="{{ route('mahasiswas.create') }}" class="btn btn-primary rounded-3 px-3 py-2 fw-semibold d-flex align-items-center gap-2 shadow-sm btn-create-hover">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-secondary">
                    <tr>
                        ...
                        <th class="py-3 border-bottom-0 text-uppercase tracking-wider small fw-bold">NIM</th>
                        <th class="py-3 border-bottom-0 text-uppercase tracking-wider small fw-bold">Nama Mahasiswa</th>
                        <th class="py-3 border-bottom-0 text-uppercase tracking-wider small fw-bold">Prodi</th>
                        <th class="py-3 border-bottom-0 text-uppercase tracking-wider small fw-bold text-center" style="width: 90px;">Smstr</th>
                ...
                <tbody class="text-dark">
                    ...
                    <tr>
                        <td class="px-3 fw-medium text-muted">{{ $mahasiswas->firstItem() + $index }}</td>
                        <td>
                        ...
                        <td>
                            <div class="fw-semibold text-dark">{{ $item->nama_mahasiswa }}</div>
                        </td>
                        <td>
                            <span class="text-dark fw-medium small">{{ $item->prodi }}</span>
                        </td>
                        <td class="text-center">
                            <span class="badge bg-secondary-subtle text-secondary-emphasis px-2 py-1 rounded-2 small fw-bold">
                                Smt {{ $item->semester }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td>
                            <span class="text-truncate d-inline-block text-muted" style="max-width: 180px;" title="{{ $item->alamat }}">
                                {{ $item->alamat }}
                            </span>
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                <a href="{{ route('mahasiswas.edit', $item) }}" class="btn btn-sm btn-icon-edit rounded-2 text-warning p-2" title="Edit Data">
                    ...
                    <tr>
                        <td colspan="9" class="text-center py-5 text-muted">
                            <div class="py-3">
                                ...
                                <span class="fw-medium">Belum ada data mahasiswa terdaftar.</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
